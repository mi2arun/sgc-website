        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#1c4c9c] rounded-lg flex items-center justify-center">
              <Bell className="w-5 h-5 text-[#f5c220]" />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-[#1c4c9c]">{title || "Announcements & Circulars"}</h2>
        ...
        <div className="bg-white rounded-xl border border-gray-200 divide-y divide-gray-100 overflow-hidden">
          {filtered.map((item, i) => (
            <Link
              key={i}
              href={item.href}
              className="flex items-center gap-4 px-5 py-4 hover:bg-[#eef3fb] transition-colors group"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="bg-[#1c4c9c]/10 text-[#1c4c9c] text-[10px] px-2 py-0.5 rounded font-medium">
                    {item.category}
                  </span>
                </div>
                <p className="text-sm font-medium text-gray-800 mt-1 group-hover:text-[#1c4c9c] transition-colors truncate">
                  {item.title}
                </p>
              </div>
              <Download className="w-4 h-4 text-gray-400 group-hover:text-[#f5c220] transition-colors shrink-0" />
            </Link>
          ))}
        </div>
      </div>
